...
class VerifiedTransfer extends Resource
{
    ...
    ## Parameters (optional):
        ...
        - scheduled [DateTime or string, default now]: date or datetime when the transfer will be processed. May be pushed to next business day if necessary. ex: "2020-11-30"
        ...
        - tags [array of strings, default []]: array of strings for reference when searching for verified transfers. ex: ["employees", "monthly"]
        - rules [array of Transfer\Rule objects, default []]: array of Transfer\Rule objects for modifying transfer behaviour. ex: [new Transfer\Rule(["key" => "resendingLimit", "value" => 5])]
    ...
    ## Attributes (return-only):
        ...
        - created [DateTime]: creation datetime for the verified transfer.
        - updated [DateTime]: latest update datetime for the verified transfer.
     */
    ...
    Send an array of VerifiedTransfer objects for creation in the Stark Bank API

    ## Parameters (required):
        - verifiedTransfers [array of VerifiedTransfer objects]: array of VerifiedTransfer objects to be created in the API
    ...
    ## Return:
        - array of VerifiedTransfer objects with updated attributes
     */
    public static function create($verifiedTransfers, $user = null)
    {
        return Rest::post($user, VerifiedTransfer::resource(), $verifiedTransfers);
    }

    private static function resource()
    {
        $verifiedTransfer = function ($array) {
            return new VerifiedTransfer($array);
        };
        return [
            "name" => "VerifiedTransfer",
            "maker" => $verifiedTransfer,
        ];
    }
}
